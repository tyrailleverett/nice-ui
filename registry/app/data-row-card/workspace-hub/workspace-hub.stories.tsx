...
export const Library: Story = {
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByRole("heading", { name: "Your workspaces" })).toBeVisible();
    ...
    await expect(canvas.getByRole("button", { name: "Open Growth experiments" })).toBeVisible();
  },
  render: () => <WorkspaceHub />,
};
...
export const SearchEmptyState: Story = {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.type(canvas.getByRole("textbox", { name: "Search workspaces" }), "unknown");
