  ...
  return (
    <TooltipProvider>
      <div className="min-h-svh bg-background text-foreground">
        <div className={cn("mx-auto flex w-full max-w-5xl flex-col gap-6 p-6 sm:p-8", className)}>
          {children}
        </div>
      </div>
    </TooltipProvider>
...
export function StatusBadge({
  ...
  return (
    <Badge
      className={cn(
        tone === "success" && "border-emerald-500/40 bg-emerald-500/10 text-emerald-700",
        tone === "warning" && "border-amber-500/40 bg-amber-500/10 text-amber-800",
        tone === "accent" && "border-violet-500/40 bg-violet-500/10 text-violet-700",
      )}
      variant="outline"
    >
...
export function FieldHint({ label }: { label: string }) {
  return (
    <Tooltip>
      <TooltipTrigger
        render={<Button aria-label={label} size="icon-xs" type="button" variant="ghost" />}
      >
        <CircleHelpIcon />
      </TooltipTrigger>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-[12.5rem_minmax(0,1fr)] sm:items-start">
      {htmlFor ? (
        ...
        <div className="flex min-h-8 items-center gap-1 pt-0.5 font-medium text-sm">{label}</div>
      )}
