...
export function SetupHelpDialog(props: DialogProps) {
  const rows = [
    [BriefcaseBusiness, "Setup guides", "Admin and onboarding references.", "Open guides"],
      ...
      >
        <div className="flex flex-col gap-4">
          {rows.map(([Icon, heading, copy, action]) => (
            <div className="flex items-center gap-4 border-b border-dashed py-4" key={heading}>
              ...
              <Button className="ml-auto shrink-0" type="button" variant="outline">
                {action}
              </Button>
            </div>
          ))}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {[
              ["Advisor office hours", "Guided setup review.", "Reserve a session"],
              ["Direct support desk", "Account and product help.", "Email support"],
              ...
              <div className="overflow-hidden rounded-xl border" key={heading}>
                ...
                <Button className="w-full rounded-none" type="button" variant="outline">
                  {action}
                </Button>
              </div>
            ))}
