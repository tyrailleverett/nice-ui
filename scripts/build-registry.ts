...
async function writeScreenshotSvg(name: (typeof screenshotNames)[number]) {
  ...
  const svgPath = path.join(rootDir, "registry/marketing/screenshot-assets", `${name}.svg`);
    ...
    items.map(async ({ baseDir, item }) => {
      const files = await Promise.all(
        (item.files ?? []).map(async (file) => {
          const relativeSourcePath = path.join(path.relative(rootDir, baseDir), file.path);
